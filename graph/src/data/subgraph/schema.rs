...
impl SubgraphDeploymentEntity {
    ...
    fn private_create_operations(self, id: &SubgraphDeploymentId) -> Vec<MetadataOperation> {
        ...
        entity.set(
            ...
            Value::from(earliest_ethereum_block_hash),
        );
        entity.set(
            "earliestEthereumBlockNumber",
            Value::from(earliest_ethereum_block_number),
        );
        entity.set(
            "latestEthereumBlockHash",
            Value::from(latest_ethereum_block_hash),
        );
        entity.set(
            "latestEthereumBlockNumber",
            Value::from(latest_ethereum_block_number),
        );
        entity.set(
            "ethereumHeadBlockHash",
            Value::from(ethereum_head_block_hash),
        );
        entity.set(
            "ethereumHeadBlockNumber",
            Value::from(ethereum_head_block_number),
        );
        entity.set("totalEthereumBlocksCount", total_ethereum_blocks_count);
        entity.set("entityCount", 0 as u64);
        entity.set(
            "graftBase",
            Value::from(graft_base.map(|sid| sid.to_string())),
        );
        entity.set("graftBlockHash", Value::from(graft_block_hash));
        entity.set("graftBlockNumber", Value::from(graft_block_number));
    ...
    pub fn unfail_operations(
        ...
        let mut entity = Entity::new();
        entity.set("failed", false);
        entity.set("health", new_health);

        // Clear the fatal error.
        entity.set("fatalError", Value::Null);
        vec![update_metadata_operation(
            Self::TYPENAME,
            id.as_str(),
            entity,
