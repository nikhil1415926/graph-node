...
impl Layout {
    ...
    // Sort the tables in the order in which they appear in the GraphQL
    // schema
    fn tables_by_position(&self) -> Vec<Arc<Table>> {
        let mut tables = self.tables.values().cloned().collect::<Vec<_>>();
        tables.sort_by_key(|table| table.position);
        tables
    }

    /// Generate the DDL for the entire layout, i.e., all `create table`
    /// and `create type` statements needed in the database schema but not
    /// the `create index` statements. Those are generated by `create_indexes_sql`
    ///
    ...
    pub fn as_ddl(&self) -> Result<String, fmt::Error> {
        ...
        // Output 'create table' statements for all tables in the order in
        // which they appear in the GraphQL schema
        for table in self.tables_by_position() {
            table.as_ddl(&mut out, self)?;
        }
        Ok(out)
    }

    pub fn create_indexes_sql(&self) -> Result<String, fmt::Error> {
        let mut out = String::new();

        for table in self.tables_by_position() {
            table.create_indexes_sql(&mut out, self)?;
        }
        Ok(out)
    }
...
impl Table {
    ...
    /// Generate the DDL for one table, i.e. the `create table` statement
    ///
    /// See the unit tests at the end of this file for the actual DDL that
    /// gets generated
    ...
    fn create_indexes_sql(&self, out: &mut String, layout: &Layout) -> fmt::Result {
        write!(out, "alter table {}\n", self.qualified_name)?;
        write!(
            out,
            "  add constraint {table}_id_{block_range}_excl\
...
mod tests {
    ...
    #[test]
    fn generate_ddl() {
        fn as_sql(layout: Layout) -> String {
            let table = layout.as_ddl().expect("Failed to generate DDL");
            let indexes = layout
                .create_indexes_sql()
                .expect("Failed to generate index SQL");
            format!("\n{}-- create indexes\n{}", table, indexes)
        }

        let layout = test_layout(THING_GQL);
        let sql = as_sql(layout);
...
-- create indexes
alter table \"rel\".\"thing\"
  add constraint thing_id_block_range_excl
      exclude using gist   (id with =, block_range with &&);
...
alter table \"rel\".\"scalar\"
  add constraint scalar_id_block_range_excl
      exclude using gist   (id with =, block_range with &&);
...
-- create indexes
alter table \"rel\".\"musician\"
  add constraint musician_id_block_range_excl
      exclude using gist   (id with =, block_range with &&);
...
alter table \"rel\".\"band\"
  add constraint band_id_block_range_excl
      exclude using gist   (id with =, block_range with &&);
...
alter table \"rel\".\"song\"
  add constraint song_id_block_range_excl
      exclude using gist   (id with =, block_range with &&);
...
alter table \"rel\".\"song_stat\"
  add constraint song_stat_id_block_range_excl
      exclude using gist   (id with =, block_range with &&);
...
type Habitat @entity {
    ...
    const FOREST_DDL: &str = "
create table rel.\"animal\" (
        \"id\"                 text not null,
        \"forest\"             text,
...
-- create indexes
alter table \"rel\".\"animal\"
  add constraint animal_id_block_range_excl
      exclude using gist   (id with =, block_range with &&);
...
alter table \"rel\".\"forest\"
  add constraint forest_id_block_range_excl
      exclude using gist   (id with =, block_range with &&);
create index attr_1_0_forest_id
    on rel.\"forest\" using btree(\"id\");

alter table \"rel\".\"habitat\"
  add constraint habitat_id_block_range_excl
      exclude using gist   (id with =, block_range with &&);
